...
router.post('/import', async (req, res) => {
    ...
    try {
        ...
        const course = await Course.findOneAndUpdate({ slug: payload.course.slug }, { ...payload.course }, { upsert: true, new: true, session });
        let totalLevels = 0;
        ...
        for (let ci = 0; ci < payload.chapters.length; ci++) {
            ...
            const chapter = await Chapter.findOneAndUpdate({ courseId: course._id, title: chData.title }, { courseId: course._id, title: chData.title, description: chData.description || '', orderIndex: ci }, { upsert: true, new: true, session });
            for (let li = 0; li < (chData.levels || []).length; li++) {
                const lvData = chData.levels[li];
                await Level.findOneAndUpdate({ chapterId: chapter._id, title: lvData.title }, {
